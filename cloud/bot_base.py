...
def get_logger(name):
    """Set up a logger for a specific bot using the new logging system"""
    # Use the new trading logger system
    return get_trading_logger(name, strategy="live_trading")
...
def log_trade(symbol, action, price, pnl=None):
    ...
    """
    try:
        # Extract coin symbol for logging
        coin = symbol.replace('USDT', '')
        
        # Use the new logging system for trade logging
        log_trade_helper(
            coin=coin,
            strategy="live_trading",
            action=action.upper(),
            price=price,
            quantity=1.0,  # Default quantity, could be made configurable
            profit_pct=pnl
        )
        
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        # Write to traditional log file for backward compatibility
