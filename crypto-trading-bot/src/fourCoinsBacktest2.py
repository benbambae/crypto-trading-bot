...
# Add parent directories to path to import logging utils
...
# Get backtest logger
logger = get_backtest_logger("multi_coin_backtest")
    ...
    for coin, entries in all_scenarios.items():
        for scenario in entries:
            ...
            try:
                ...
                # Use the new backtest result logging
                if 'Total Return (%)' in metrics and 'Win Rate (%)' in metrics:
                    log_backtest_result(
                ...
                logger.error(f"❌ Error in {coin}-{strategy_name}: {str(e)}")
